...
- **Dropzone composes with Empty** — `DropzoneGroup`, `DropzoneTitle`, `DropzoneDescription` and `DropzoneUploadIcon` are gone. Dropzone now only covers the interactive parts; build the zone's visual content with shadcn/ui's [Empty](https://ui.shadcn.com/docs/components/empty) component instead.
